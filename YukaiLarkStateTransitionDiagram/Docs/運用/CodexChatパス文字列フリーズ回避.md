...
- 修正パッチが確認できるまでは、Markdown 表示切替を避ける。
- 危険文字列を含む過去会話は、表示切替時の再描画で落ちる可能性がある。
- 長いログやスタックトレースは Chat 本文に出さず、運用メモや報告ドラフトへ退避する。
- Visual Codex Studio 1.2.1 では、2026-07-02 時点でこのクラッシュ修正版を確認できていない。
